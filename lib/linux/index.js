...
function parseDisplaysOutput(out) {
  return out
    ...
    .filter(line => line.search(/\dx\d/))
    .map((line, ix) => {
      const
        parts = line.split(' ')
        name = parts[0],
        primary = parts[2] === 'primary',
        crop = primary ? parts[3] : parts[2],
        resParts = crop.split(/[x+]/),
        width = +resParts[0],
        height = +resParts[1],
        offsetX = +resParts[2],
        offsetY = +resParts[3]
...
function linuxSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    const screenFilt = options.screen ? { name: options.screen } : { primary: true }

    listDevices().then((screens) => {
      const screen = screens.find(
        options.screen ?
        screen => screen.id === options.screen :
        screen => screen.primary)

      exec(`import -silent -window root -crop ${screen.crop} -screen jpeg:- `, {
        encoding: 'buffer',
