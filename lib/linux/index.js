...
function listDisplays() {
  return new Promise((resolve, reject) => {
    execFile("xrandr", ["--current"], (err, stdout) => {
      if (err) {
        return reject(err);
      }
      ...
      const screen =
        screens.find(
          options.screen
            ...
            : (screen) => screen.primary || screen.id === "default"
        ) || screens[0];
      ...
      execFile(
        ...
        (err, stdout) => {
          if (err) {
            ...
            return resolve(
              options.filename ? path.resolve(options.filename) : stdout
            );
          }
        }
      );
