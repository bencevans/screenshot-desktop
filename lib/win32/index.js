...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    execFile('"' + path.join(unpackedDirectory, 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice, { cwd: unpackedDirectory, shell:true
    }, (err, stdout) => {
      if (err) {
        return reject(err)
      } else {
...
function listDisplays () {
  return new Promise((resolve, reject) => {
    let unpackedDirectory = __dirname.replace('app.asar', 'app.asar.unpacked')
    execFile('"' + path.join(unpackedDirectory, 'screenCapture_1.3.2.bat') + '" /list', {cwd: unpackedDirectory, shell: true},
      (err, stdout) => {
        if (err) {
          return reject(err)
        }
