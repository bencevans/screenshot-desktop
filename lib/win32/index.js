...
// Escape cmd.exe metacharacters (spaces, parens, etc.) with ^ so paths
// like "C:\Path With Space\file.bat" work correctly with cmd.exe /c
...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const cmdParts = [escapeCmdMeta(tmpBat), escapeCmdMeta(imgPath)]

    if (displayName) {
      cmdParts.push('/d', displayName)
    }
    execFile('cmd.exe', ['/c', cmdParts.join(' ')], {
...
function listDisplays () {
  return new Promise((resolve, reject) => {
    const tmpBat = copyToTemp()
    execFile('cmd.exe', ['/c', escapeCmdMeta(tmpBat) + ' /list'], {
        cwd: path.join(os.tmpdir(), 'screenCapture')
      },
      (err, stdout) => {
        if (err) {
