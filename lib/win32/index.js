...
const { exec } = require('child_process') //ES6 Import for electron purposes
...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    /**
     * Due to limitations of electron, windows cannot execute out of app.asar
     * bat file will be in the unpacked directory alongside the binary
     */
    let unpackedDirectory = __dirname.replace('app.asar', 'app.asar.unpacked')

    exec('"' + path.join(unpackedDirectory, 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice, { cwd: unpackedDirectory
    }, (err, stdout) => {
      if (err) {
        return reject(err)
      } else {
...
function listDisplays () {
  return new Promise((resolve, reject) => {
    let unpackedDirectory = __dirname.replace('app.asar', 'app.asar.unpacked')
    exec(
      '"' + path.join(unpackedDirectory, 'screenCapture_1.3.2.bat') + '" /list', {
