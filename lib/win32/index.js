...
function copyToTemp () {
  ...
  const includeBat = path
    .join(__dirname, 'screenCapture_1.3.2.bat')
    .replace('app.asar', 'app.asar.unpacked')
  const includeManifest = path
    .join(__dirname, 'app.manifest')
    .replace('app.asar', 'app.asar.unpacked')
...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    execFile(tmpBat, [imgPath, ...displayChoice],
      {
...
function parseDisplaysOutput (output) {
  ...
  return output
    ...
    .map((s) => s.replace(/[\n\r]/g, ''))
    .map((s) =>
      s.match(/(.*?);(.?\d+);(.?\d+);(.?\d+);(.?\d+);(.?\d*[\.,]?\d+)/) // eslint-disable-line
    )
    .filter((s) => s)
    .map((m) => ({
      id: m[1],
