...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const width = options.width ? parseInt(options.width) : 0
    const height = options.height ? parseInt(options.height) : 0
    ...
    //Pass custom resolution

    if(width != 0 && height != 0){
      batArgs.push('/rw', width)
      batArgs.push('/rh', height)
    }
