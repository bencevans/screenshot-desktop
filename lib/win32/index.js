...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    exec('"' + path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice, { cwd: __dirname.replace('app.asar', 'app.asar.unpacked') 
    }, (err, stdout) => {
      if (err) {
        return reject(err)
      } else {
