...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    let format = (options.format || 'jpg').toLowerCase()
    if (!allowedFormats.includes(format)) {
      return reject(new Error('Invalid format'))
    }

    // Sanitize filename
    const safeFilename = options.filename ? options.filename.replace(/[^a-zA-Z0-9._\-/]/g, '') : ''
    const tmpPath = temp.path({
      suffix: `.${format}`
    })
    const imgPath = path.resolve(safeFilename || tmpPath)

    // Sanitize displayName
    const displayName = options.screen ? String(options.screen).replace(/[^a-zA-Z0-9._\-/]/g, '') : ''
    ...
    const args = [imgPath]
    if (displayName) {
      args.push('/d', displayName)
    }
    require('child_process').execFile(tmpBat, args, {
